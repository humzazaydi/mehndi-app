...
@Component({
  ...
  template: `
    <mat-toolbar class="sticky top-0 z-50 !h-16 !px-3 sm:!px-6 !bg-[var(--mehndi-nav)] backdrop-blur-xl border-b border-[var(--mehndi-border)] shadow-[0_10px_30px_rgba(15,61,46,0.08)]">
      <div class="flex items-center gap-3 flex-1 min-w-0">
        <a routerLink="/" class="flex items-center gap-2 no-underline min-w-0">
          <div class="w-10 h-10 brand-gradient rounded-2xl flex items-center justify-center shadow-lg shrink-0 ornate-border">
      ...
      <nav class="hidden md:flex items-center gap-1 text-[var(--mehndi-heading)]">
        <a mat-button routerLink="/artists" routerLinkActive="!text-[var(--mehndi-gold)]">Artists</a>
        <a mat-button routerLink="/packages" routerLinkActive="!text-[var(--mehndi-gold)]">Packages</a>
        <a mat-button routerLink="/store" routerLinkActive="!text-[var(--mehndi-gold)]">Henna Store</a>
    ...
    <footer class="relative overflow-hidden text-[var(--mehndi-cream)] py-10 mt-0"
            style="background:linear-gradient(135deg,var(--mehndi-action-deep),#09251c 72%)">
      ...
      <div class="page-container relative grid grid-cols-1 md:grid-cols-3 gap-8">
        <div>
          <h3 class="text-[var(--mehndi-gold-soft)] font-semibold mb-3" style="font-family:'Playfair Display',serif">Mehndi Studio</h3>
          <p class="text-sm leading-relaxed opacity-85">Premium bridal henna services in Karachi with refined booking, transparent packages, and artist-led care.</p>
        </div>
        <div>
          ...
          <nav class="flex flex-col gap-2 text-sm">
            ...
            <a routerLink="/store" class="hover:text-[var(--mehndi-gold-soft)] transition-colors">Henna Cone Store</a>
      ...
      <div class="relative border-t border-[rgba(234,215,162,0.18)] mt-8 pt-6 text-center text-xs opacity-65">
        Copyright {{ year }} Mehndi Studio. All rights reserved.
      </div>
    </footer>
  `,
})
