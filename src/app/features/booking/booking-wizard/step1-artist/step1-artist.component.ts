      ...
      @if (loading()) {
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-8">
          @for (artist of artists(); track artist.id) {
            <div class="border-2 rounded-lg p-4 cursor-pointer transition-all flex items-center gap-4 bg-[var(--mehndi-panel-soft)]"
                 [class.border-[var(--mehndi-gold)]]="wizard.data().artistId === artist.id"
                 [class.bg-[rgba(245,158,11,0.12)]]="wizard.data().artistId === artist.id"
              ...
              @if (wizard.data().artistId === artist.id) {
                <mat-icon class="text-[var(--mehndi-success)] shrink-0">check_circle</mat-icon>
              }
            </div>
          }
        </div>
        ...
        @if (wizard.data().artistId && availablePackages().length > 0) {
          ...
          <div class="space-y-3">
            @for (pkg of availablePackages(); track pkg.id) {
              <div class="border-2 rounded-lg p-4 cursor-pointer transition-all bg-[var(--mehndi-panel-soft)]"
                   [class.border-[var(--mehndi-gold)]]="wizard.data().packageId === pkg.id"
                   [class.bg-[rgba(245,158,11,0.12)]]="wizard.data().packageId === pkg.id"
                ...
                <div class="flex items-center justify-between gap-3">
                  ...
                  <div class="text-right">
                    ...
                    @if (wizard.data().packageId === pkg.id) {
                      <mat-icon class="text-[var(--mehndi-success)]">check_circle</mat-icon>
                    }
                  </div>
                </div>
              </div>
